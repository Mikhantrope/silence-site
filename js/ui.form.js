(function (global) {
  ...
  function wireForm(form){var C=global.SILENCE_CORE,lang=C.getLang(),t=function(k){return C.t(k,lang);};var phone=form.querySelector('#f-phone'),err=form.querySelector('[data-phone-error]'),status=form.querySelector('[data-form-status]'),hp=form.querySelector('#f-company');if(phone)attachPhoneMask(phone);
    form.addEventListener('submit',function(e){e.preventDefault();status.removeAttribute('data-ok');status.removeAttribute('data-err');status.textContent='';
      ...
      var problem=form.querySelector('#f-problem'),surface=form.querySelector('#f-surface'),system=form.querySelector('#f-system'),room=form.querySelector('#f-room'),area=form.querySelector('#f-area'),name=form.querySelector('#f-name'),email=form.querySelector('#f-email'),ads=form.querySelector('#f-ads');
      var fields=[['Проблема',labelOf(problem)],['Поверхность',labelOf(surface)],['Система',system?system.value:''],['Тип помещения',room?room.value:''],[t('fName'),name?name.value:''],[t('fPhone'),'+7 '+formatPhone(digits)],['Email',email?email.value:''],[t('fArea'),area?area.value:''],['Согласие на обработку',policy&&policy.checked?'Да':'Нет'],['Согласие на рассылку',ads&&ads.checked?'Да':'Нет'],['Страница',global.location.href]].concat(utmFields());
