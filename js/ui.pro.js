/* SILENCE — ui.pro.js
   pro.html: переключатель аудитории (SITE.audience: eng/biz/dec) с задачами
   и подборками, полные техданные ГСП/ЦСП (CATALOG.boards[].detail — только
   здесь), и справочные карточки по нормативам (SILENCE_NORMS). Названия
   систем и id нигде не дублируются вручную — только через core.findItem(). */
(function (global) {
  'use strict';
  ...
  function renderAudience(host, key) {
    ...
    copy.tasks.forEach(function (task) {
      ...
      if (!item) return; /* защитный отказ: не выдумываем ссылку, если позиции нет */
      var li = C.el('li', {}, [
  ...
  function render(node) {
    ...
    AUD_ORDER.forEach(function (key) {
      ...
      btn.addEventListener('click', function () {
        ...
        if (global.history && global.history.replaceState) global.history.replaceState(null, '', '#aud-' + key);
      });
    ...
    /* Нормативы — самостоятельный подраздел, а не название всей страницы. */
    ...
    /* Initial browser hash scrolling happens before this JS-rendered content
       exists, so repeat it once after rendering for #pro-norms/materials. */
    if (/^#pro-(audiences|norms|materials)$/.test(global.location.hash || '')) {
      var target = document.querySelector(global.location.hash);
      if (target) global.requestAnimationFrame(function () { target.scrollIntoView({ block: 'start' }); });
    }
  }
