# SILENCE v16 — запуск и настройка
...
- подборщик теперь учитывает тип шума, а не только поверхность;
- сценарий «соседи за стеной» корректно ведёт на облицовки стены;
- входящий шум снизу отделён от задачи снижения собственного ударного шума вниз;
- напольные решения с `ΔLn,w` показываются только для сценария передачи ударного шума к соседям снизу;
- эхо вынесено в отдельную ветку акустики помещения с декоративно-акустическими панелями;
- удалён дублирующий блок сценариев на главной;
- мобильный разрез вместо шести длинных карточек использует пять компактных переключателей и одну карточку деталей;
- новые сценарии и контекст акустики передаются в форму заявки.
