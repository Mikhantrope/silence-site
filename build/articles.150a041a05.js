...
function close(focus) { panel.hidden = true; button.setAttribute('aria-expanded', 'false'); button.setAttribute('aria-label', 'Открыть меню'); if (focus)
...
window.addEventListener('resize', function () { if (window.innerWidth >= 1180)
close(false); });
}
...
theme.addEventListener('click', function () { var t = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark'; document.documentElement.dataset.theme = t; try {
localStorage.setItem('silence.theme', t);
}
catch (ignore) { } themeState(); });
themeState();
}
...
function apply() { var q = normalize(input.value), n = 0; cards.forEach(function (c) { var visible = (!active || c.dataset.category === active) && (!q || normalize(c.textContent).includes(q)); c.hidden = !visible; if (visible)
n++; }); buttons.forEach(function (b) { b.setAttribute('aria-pressed', String(b.dataset.articleFilter === active)); }); empty.hidden = n !== 0; count.textContent = 'Показано: ' + n + ' из ' + cards.length; }
input.addEventListener('input', apply);
buttons.forEach(function (b) { b.addEventListener('click', function () { active = b.dataset.articleFilter; apply(); }); });
var reset = document.querySelector('[data-article-reset]');
if (reset)
reset.addEventListener('click', function () { active = ''; input.value = ''; apply(); input.focus(); });
